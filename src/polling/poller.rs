...
impl Poller {
    ...
    /// Run one poll cycle across all providers in parallel.
    /// Uses async-compat to bridge reqwest's tokio requirement into smol.
    pub async fn poll_once(&self) -> PollResult {
        ...
        let review_futures: Vec<_> = self
            ...
            .map(|(provider, user)| {
                ...
                let user = user.clone();
                async move {
                    let result = provider.list_reviewing(&user).compat().await;
        ...
        let authored_futures: Vec<_> = self
            ...
            .map(|(provider, user)| {
                ...
                let user = user.clone();
                async move {
                    let result = provider.list_authored(&user).compat().await;
                    (provider.name().to_string(), result)
                }
            })
            .collect();
        ...
        let (review_results, authored_results) =
            futures::join!(join_all(review_futures), join_all(authored_futures));

        for (name, result) in review_results {
            match result {
                Ok(prs) => all_reviewing.extend(prs),
                Err(e) => errors.push(PollError::ProviderFailed {
                    provider: name,
                    message: e.to_string(),
                }),
            }
